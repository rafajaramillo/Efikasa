// Este archivo muestra las 6 estadísticas estrictamente solicitadas sobre consumo energético
...
class _PestanaMasConsumoState extends State<PestanaMasConsumo> {
  ...
  // Encabezados HTTP necesarios para autenticación básica con InfluxDB
  ...
  // Esta función calcula la energía consumida (en kWh) a partir de valores de potencia (W) y tiempo (s)
// Fórmula: Energía (kWh) = (Potencia * Tiempo) / 3600000
// Donde el tiempo está en segundos y la potencia en watts
  ...
  Future<Map<String, double>> obtenerConsumo(String rango) async {
  ...
  // Devuelve una lista de identificadores de entidades cuya métrica termina en '_power'
  ...
  Future<Map<String, double>> obtenerAgrupadoPorHoraDelMesActual() async {
    final ahora = DateTime.now();
    final inicio = DateTime.utc(ahora.year, ahora.month, 1);
    final fin = ahora.toUtc();

    final queryStr =
        "SELECT mean(\"value\") FROM \"W\" WHERE time >= '${inicio.toIso8601String()}' AND time < '${fin.toIso8601String()}' AND \"entity_id\" =~ /.*_power/ GROUP BY time(1h) fill(null)";
    ...
    for (var serie in series) {
      ...
      for (int i = 1; i < values.length; i++) {
        ...
        if (potencia != null) {
          ...
          final clave = t1.toIso8601String().substring(11, 16);
          agrupado[clave] = (agrupado[clave] ?? 0) + energia;
        }
      }
    }
    return agrupado;
  ...
  // Esta función ejecuta una consulta con la función `integral()` de InfluxDB
// `integral(value) / 3600000` convierte el área bajo la curva de potencia en energía (kWh)
  ...
  Future<Map<String, double>> obtenerAgrupadoPorDiaDeSemana() async {
    final ahora = DateTime.now();
    final inicioSemana =
        ahora.toUtc().subtract(Duration(days: ahora.weekday - 1));
    final finSemana = inicioSemana.add(const Duration(days: 7));

    final queryStr =
        "SELECT \"value\" FROM \"W\" WHERE time >= '${inicioSemana.toIso8601String()}' AND time < '${finSemana.toIso8601String()}' AND \"entity_id\" =~ /.*_power/ GROUP BY \"entity_id\"";
    ...
    for (var serie in series) {
      ...
      for (int i = 1; i < values.length; i++) {
        ...
        if (potencia != null) {
          final energia = (potencia * duracionSeg) / 3600000;
          final clave = t1.toIso8601String().substring(0, 10);
          consumoPorDia[clave] = (consumoPorDia[clave] ?? 0) + energia;
        }
      }
    }
  ...
  // Convierte una cadena de fecha ISO a un formato legible: 'día de mes de año' en español
  ...
  Widget build(BuildContext context) {
    // Construcción del widget principal que muestra las estadísticas
    final ahora = DateTime.now();
    ...
    // FutureBuilder que espera múltiples resultados de forma concurrente
    return FutureBuilder(
      future: Future.wait([
        obtenerTotalConsumoPorMes(),
        obtenerConsumo("'$inicioHoy'"),
        obtenerConsumo("'$inicioMesAnterior' AND time < '$inicioMesActual'"),
        obtenerAgrupadoPorHoraDelMesActual(),
        obtenerAgrupadoPorDiaDeSemana(),
      ...
      builder: (context, snapshot) {
        ...
        // Indicador simple mientras se cargan los datos
        ...
        // Extracción de resultados de cada futuro completado
        ...
        final diasSemana = datos[4] as Map<String, double>;
        final diasMes = datos[5] as Map<String, double>;

        // Determinación de los valores máximos para cada categoría
        final mesMax = meses.entries.isNotEmpty
            ? meses.entries.reduce((a, b) => a.value > b.value ? a : b)
            : null;
        final maxHoy = hoy.entries.isNotEmpty
            ? hoy.entries.reduce((a, b) => a.value > b.value ? a : b)
            : null;
        final maxMesAnterior = mesAnterior.entries.isNotEmpty
            ? mesAnterior.entries.reduce((a, b) => a.value > b.value ? a : b)
            : null;
        final horaPico = horasMes.entries.isNotEmpty
            ? horasMes.entries.reduce((a, b) => a.value > b.value ? a : b)
            : null;
        final diaSemanaMax = diasSemana.entries.isNotEmpty
            ? diasSemana.entries.reduce((a, b) => a.value > b.value ? a : b)
            : null;
        final diaMesMax = diasMes.entries.isNotEmpty
            ? diasMes.entries.reduce((a, b) => a.value > b.value ? a : b)
            : null;

        // Muestra cada estadística usando tarjetas informativas
        return ListView(
          padding: const EdgeInsets.all(16),
          children: [
            buildStatCard(
              'Mes de mayor consumo ($anio)',
              mesMax != null
                  ? '${mesMax.key} - ${mesMax.value.toStringAsFixed(2)} kWh'
                  : 'Sin datos registrados en ese periodo',
            ),
            buildStatCard(
              'Dispositivo que más consumió hoy',
              maxHoy != null
                  ? '${maxHoy.key.replaceAll('_power', '')} - ${maxHoy.value.toStringAsFixed(2)} kWh'
                  : 'Sin datos registrados en ese periodo',
            ),
            buildStatCard(
              'Dispositivo que más consumió en el mes anterior',
              maxMesAnterior != null
                  ? '${maxMesAnterior.key.replaceAll('_power', '')} - ${maxMesAnterior.value.toStringAsFixed(2)} kWh'
                  : 'Sin datos registrados en ese periodo',
            ),
            buildStatCard(
              'Horas pico de mayor consumo ($mesActualNombre)',
              horaPico != null
                  ? '${horaPico.key} - ${horaPico.value.toStringAsFixed(2)} kWh'
                  : 'Sin datos registrados en ese periodo',
            ),
            buildStatCard(
              'Día de mayor consumo (semana actual)',
              diaSemanaMax != null
                  ? '${formatearFecha(diaSemanaMax.key)} - ${diaSemanaMax.value.toStringAsFixed(2)} kWh'
                  : 'Sin datos registrados en ese periodo',
            ),
            buildStatCard(
              'Día de mayor consumo ($mesActualNombre)',
              diaMesMax != null
                  ? '${formatearFecha(diaMesMax.key)} - ${diaMesMax.value.toStringAsFixed(2)} kWh'
                  : 'Sin datos registrados en ese periodo',
            ),
          ],
        );
