...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Future<void> fetchDevices() async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        devices = deviceList;
        // Si no había ninguno seleccionado o el actual no existe, elige el primero
        if (!devices.contains(selectedDevice)) {
          selectedDevice = devices.isNotEmpty ? devices.first : selectedDevice;
          fetchConsumptionData();
        }
      });
    }
  ...
  Future<void> fetchConsumptionData() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (data.isNotEmpty) {
        ...
        // Antes de llamar setState, chequea mounted:
        if (!mounted) return;

        setState(() {
          consumoData = parsedData;
          if (consumoData.isNotEmpty) {
            final double peak = consumoData
                .map((e) => e["power"] as double)
                .reduce((a, b) => a > b ? a : b);
            maxY = ((peak / 10).ceil() * 10).toDouble();
          } else {
